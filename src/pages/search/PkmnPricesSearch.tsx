...
export default function PkmnPricesSearch() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/pkmn-cards/search?${params}`, {
      signal: controller.signal,
    })
      .then(async (response) => {
        if (!response.ok) throw new Error("Could not search the new catalogue");
  ...
  return (
    <main className="pkmn-search">
      <div className="pkmn-search__header">
        <h1>New card catalogue</h1>
        <p>Search the cards saved in the PkmnPrices trial database.</p>
      </div>
      <form className="pkmn-search__form" onSubmit={handleSearch}>
        <label htmlFor="pkmn-card-name">Card name</label>
